...
void human() {
	...
	vz = 0.0;
	d = sqrt(vy * vy + vz * vz);
	glRotatef(-1 * atan2(vy / d, vz/d)*PA1, 1, 0, 0); 
	glRotatef(-1 * atan2(-1 * vx, d)*PA1, 0, 1, 0);
	std::cout << atan2(vy /d, vz/d) * PA1 << " " << atan2(-1 * vx, d) * PA1 << std::endl;
	//part for tranform axis from 
	glBegin(GL_LINES);
