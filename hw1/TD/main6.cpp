...
void human() {
	...
	glPushMatrix(); //to right shoulder joint
		...
		*/
		...
		//glRotatef(180, 1, 1, 0); //rotating of joint
		//In parent local system, parent want child's z-axis becomes axis defined parents
		glRotatef(-1.0*getAlpha(-1, 0, 0, 1), 1, 0, 0); //transformation for axis matching. rightarm1 local base vector equals to body local base
		glRotatef(-1.0*getBeta(-1, 0, 0, 1), 0, 1, 0);
